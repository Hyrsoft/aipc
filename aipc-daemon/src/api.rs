...
async fn ai_projects(State(state): State<AppState>) -> Result<impl IntoResponse, AppError> {
    Ok(Json(state.ai.list_projects().await?))
}

async fn ai_project_create(
...
async fn ai_models(State(state): State<AppState>) -> Result<impl IntoResponse, AppError> {
    Ok(Json(state.ai.list_models().await?))
}

async fn ai_model_upload(
    State(state): State<AppState>,
...
async fn ai_osd(State(state): State<AppState>) -> impl IntoResponse {
