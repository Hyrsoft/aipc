    ...
    int RetinaFaceEngine::inference(const cv::Mat &img, std::vector<ObjectDet> &results) {
        ...
        for (int i = 0; i < num_priors; i++) {
            ...
            if (face_score > 0.5) {
                ...
                float box_x = (deqnt_affine_to_f32(bbox[0], loc_zp, loc_scale)) * VARIANCES[0] * prior_ptr[i][2] +
                              prior_ptr[i][0];
                float box_y = (deqnt_affine_to_f32(bbox[1], loc_zp, loc_scale)) * VARIANCES[0] * prior_ptr[i][3] +
                              prior_ptr[i][1];
                float box_w = (float) expf((deqnt_affine_to_f32(bbox[2], loc_zp, loc_scale)) * VARIANCES[1]) *
                              prior_ptr[i][2];
                float box_h = (float) expf((deqnt_affine_to_f32(bbox[3], loc_zp, loc_scale)) * VARIANCES[1]) *
                              prior_ptr[i][3];
                ...
                for (int j = 0; j < 5; j++) {
                    landms_fp32[i * 10 + 2 * j] = deqnt_affine_to_f32(landms[i * 10 + 2 * j], landms_zp, landms_scale) *
                                                          VARIANCES[0] * prior_ptr[i][2] +
                                                  prior_ptr[i][0];
                    landms_fp32[i * 10 + 2 * j + 1] =
                            deqnt_affine_to_f32(landms[i * 10 + 2 * j + 1], landms_zp, landms_scale) * VARIANCES[0] *
                                    prior_ptr[i][3] +
                            prior_ptr[i][1];
                }
        ...
        for (int i = 0; i < validCount; ++i) {
            ...
            ObjectDet det;
            det.box = cv::Rect((int) (clamp(x1, 0, width) * scaleX), (int) (clamp(y1, 0, height) * scaleY),
