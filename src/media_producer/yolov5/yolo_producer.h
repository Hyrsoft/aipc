 ...
 *
 * 数据流架构（对齐 luckfox_pico_rtsp_yolov5 例程）：
 *   VI --(手动 GetChnFrame)-->
 *        NV12 帧
 *           |
 *           v (NV12 → BGR, OpenCV)
 *        BGR 帧 --> letterbox --> NPU 推理 --> 检测结果
 *           |
 *           v (OSD 叠加, OpenCV rectangle/putText)
 *        RGB 帧 (带框)
 *           |
 *           v (手动 SendFrame)
 *        VENC --> 编码流
 ...
 * 特点：
 * - 无 VPSS，与例程一致
 * - VI u32Depth=2，支持手动 GetChnFrame
 * - 支持 YOLOv5 目标检测
 * - 检测框通过 OpenCV 叠加到编码流
 * - DDR 带宽受限，推荐 480p
 *
...
namespace media {
    ...
    private:
        ...
        /**
         * @brief 初始化 MPI 组件（ISP/VI/VENC，无 VPSS）
         */
        int InitMpi();
        ...
        /**
         * @brief 初始化 RGB 缓冲池（与例程一致：1 block 预分配）
         */
        bool InitRgbPool();
         ...
         */
        bool ProcessFrame(void *nv12_data, int width, int height, int stride, void *rgb_output, uint64_t pts);
