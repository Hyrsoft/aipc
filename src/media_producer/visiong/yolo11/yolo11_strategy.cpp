/**
 * @file yolo11_strategy.cpp
 * @brief YOLO11 物体检测 - 推理 + OSD 绘制
 */

#include "yolo11_strategy.h"
...
Yolo11Strategy::Yolo11Strategy(const std::string& model_path, const std::string& label_path,
...
std::unique_ptr<NPU> Yolo11Strategy::CreateNPU() {
    auto npu = std::make_unique<NPU>(
        ModelType::YOLO11, model_path_, label_path_, conf_thresh_, nms_thresh_);
    if (!npu->is_initialized()) {
        LOG_ERROR("YOLO11 NPU init failed: model={}", model_path_);
        return nullptr;
    }
    return npu;
}

ImageBuffer Yolo11Strategy::ProcessFrame(const ImageBuffer& frame, NPU* npu) {
