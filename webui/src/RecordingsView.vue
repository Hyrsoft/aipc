...
async function control(action: 'start' | 'stop') { busy.value = true; error.value = ''; try { status.value = await api.recordingControl(action); notice.value = action === 'start' ? '正在等待关键帧开始录像' : '正在完成 MP4 文件'; await refresh() } catch (cause) { error.value = String(cause) } finally { busy.value = false } }
...
<template>
  <div class="recordings-page">
    ...
    <section class="panel recordings-list">
      ...
      <div class="recording-table"><div class="recording-row recording-header"><span></span><span>文件</span><span>录制时间</span><span>时长</span><span>大小</span><span>画面</span><span>操作</span></div><div v-for="file in files" :key="file.id" class="recording-row"><input type="checkbox" :checked="selected.includes(file.id)" @change="toggle(file.id)"><button class="file-button" @click="playing = file">{{ file.file_name }}</button><span>{{ new Date(file.created_at_ms).toLocaleString() }}</span><span>{{ duration(file.duration_ms) }}</span><span>{{ bytes(file.bytes) }}</span><span>{{ file.width }}×{{ file.height }} · {{ file.fps }}fps</span><span class="row-actions"><button class="accent compact" @click="playing = file">播放</button><a :href="`/api/v1/recordings/${encodeURIComponent(file.id)}/download`">下载</a></span></div><p v-if="!files.length" class="empty-state">还没有已完成的录像</p></div>
    </section>
